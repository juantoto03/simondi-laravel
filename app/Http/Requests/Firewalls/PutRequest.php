...
class PutRequest extends FormRequest
{
    ...
    {
        return [
            'fir_nombre' => 'required|min:1|unique:firewalls,fir_nombre,' . $this->route('firewall')->id,
            'fir_serial' => 'required|min:1|unique:firewalls,fir_serial,' . $this->route('firewall')->id,
            'fir_memoria' => 'required|min:1',
            'fir_cpu' => 'required|min:1',
            'fir_puertos' => 'required|min:1',
            'fir_macadd' => 'required|min:1',
            'fir_ip' => 'required|min:1|unique:firewalls,fir_ip,' . $this->route('firewall')->id,
